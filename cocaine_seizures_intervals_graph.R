...
# Create a new column in the data frame that represents the four-year interval 
# for each year
complete_data$year_group <- cut(complete_data$year, 
                                breaks = seq(min(complete_data), 
                                             max(complete_data$year), 
                                             by = 5))
...
agg_plot <- ggplot(data_agg, aes(x=year_group, y=seizure, fill=year_group)) + 
  ...
  scale_fill_manual(values=c("orange", "darkorange", "steelblue", "navy"), 
                    labels=c("1996-2001", "2001-2006", "2006-2011", 
                             "2011-2016")) + 
...
selected_vars <- c("year", "un1961", "un1971", "un1988", "wholesale", "retail", 
                   "seizure", "seizure_log")
