...
worldbank_data <- wb_data(indicator = c("NY.GDP.PCAP.CD",
                                        "NE.DAB.TOTL.CD", "NE.DAB.TOTL.ZS",
                                        "NY.ADJ.NNTY.PC.CD",
                                        "NE.IMP.GNFS.ZS", "NE.IMP.GNFS.CD",
...
merged_wb_data <- merge(worldbank_data, 
                        y = wb_countries[c("iso3c", "country", 
                                           "region", "region_iso3c", 
...
merged_wb_data <- merged_wb_data %>%
  ...
  select(region, region_iso3c, country, code, year, gdp_cap_dollar, gne_dollar, 
         gne_gdp, adjusted_nicap_dollar,
...
us_deflator_data <- subset(
  us_deflator_data, select = -c(iso3c, indicatorID, indicator, iso2c, country))
...
merged_data <- merged_data[, c("region", "region_iso3c",
                               "country", "code", "year", 
...
print(head(merged_data$adjusted_gdp))
...
# Reorder columns
merged_data <- merged_data[, c("region", "region_iso3c",
                               "country", "code", "year", "log_adjusted_gdp",
                               "adjusted_gdp", "deflator_us", "gdp_cap_dollar",
                               "gne_dollar", "gne_gdp", 
                               "adjusted_nicap_dollar", 
                               "income_level", "income_level_iso3c",
                               "lending_type", "lending_type_iso3c", 
...
write.csv(
  merged_data, 
  file = "~/Desktop/working-sessions/cleaning_data/government_data.csv", 
  row.names = FALSE)
