...
# ANOVA test: This test compares the means of a continuous response variable across different levels of one or more categorical predictor variables.
...
gdp_retail_seizure <- ggplot(complete_data, aes(x = gdp_euro_log, y = retail, color = seizure_log)) +
  geom_point() +
...
# Create a scatter plot with GDP, retail prices and cocaine seizures
gni_retail_seizure <- ggplot(complete_data, aes(x = gnicap_log, y = retail, color = seizure_log)) +
  geom_point() +
...
gdp_gni_retail_seizure <- ggarrange(gdp_retail_seizure, gni_retail_seizure + 
